<div class="p-6">
            ...
            @if($board->taskLists)
                @foreach($board->taskLists as $taskList)
                    <div class="flex-none w-80 bg-white rounded-lg border border-gray-200 shadow-sm">
                        ...
                        <!-- Tasks Container -->
                        <div class="p-4 space-y-3 min-h-[200px]" data-list-id="{{ $taskList->id }}">
                            @if($taskList->tasks)
                                @foreach($taskList->tasks as $task)
                                    <div class="bg-white border border-gray-200 rounded-lg p-3 shadow-sm cursor-move hover:shadow-md transition-shadow" 
                                         data-task-id="{{ $task->id }}">
                                        <div class="flex items-start justify-between">
                                            <div class="flex items-start space-x-2 flex-1">
                                                <!-- Completion Checkbox -->
                                                <button wire:click="$dispatch('toggle-task-complete', { taskId: {{ $task->id }} })" 
                                            ...
                                            <div class="flex items-center space-x-1 opacity-0 group-hover:opacity-100 transition-opacity">
                                                <button class="p-1 text-gray-400 hover:text-gray-600 rounded text-sm">
                                                    ✏️
                                                </button>
                                                <button wire:click="$dispatch('delete-task', { taskId: {{ $task->id }} })"
    ...
    <script>
        ...
        function initializeSortable() {
            document.querySelectorAll('[data-list-id]').forEach(function(list) {
